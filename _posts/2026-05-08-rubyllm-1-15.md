...
A chat with ten messages can tell you the total. An agent can tell you the total. A generated image can tell you the total. No spreadsheet. No copying provider pricing into app code.
...
`tokens.input` now means normal input tokens. Cache reads and cache writes are separate.
...
class Weather < RubyLLM::Tool
  ...
  param :latitude, desc: "Latitude"
  param :longitude, desc: "Longitude"
...
The old `on_*` callbacks still work, but they warn. They'll go away in RubyLLM 2.0.
...
- The new `acts_as` API follows Rails association inference more closely.
